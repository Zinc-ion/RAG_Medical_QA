...
def retrieve_metadata():
    ...
    vars2readme = {}
    try:
        with open("lightragPkg/__init__.py") as f:
        ...
        raise FileNotFoundError("Metadata file './lightragPkg/__init__.py' not found.")
...
def read_api_requirements():
    api_deps = []
    try:
        with open("lightragPkg/api/requirements.txt") as f:
...
def read_extra_requirements():
    api_deps = []
    try:
        with open("lightragPkg/tools/lightrag_visualizer/requirements.txt") as f:
            api_deps = [line.strip() for line in f if line.strip()]
    except FileNotFoundError:
        print("Warning: API requirements.txt not found.")
    return api_deps
...
setuptools.setup(
    ...
    entry_points={
        "console_scripts": [
            "lightragPkg-server=lightragPkg.api.lightrag_server:main [api]",
            "lightragPkg-viewer=lightragPkg.tools.lightrag_visualizer.graph_visualizer:main [tools]",
        ],
    },
)
